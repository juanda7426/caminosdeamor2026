								...
								return (
									<div className='col-lg-4 col-md-6' key={plan.id}>
										<div className='plan-trigger-card' onClick={() => openModal(plan)}>
											<div className={`plan-header-simple`} style={{ backgroundColor: plan.color || "#0087B7" }}>
												<h3 className='plan-title-trigger text-white'>{plan.name}</h3>
			...
			{activePlan && (
				<div className='modal-backdrop-custom' onClick={closeModal}>
					...
					<div className='modal-content-custom' onClick={(e) => e.stopPropagation()}>
						...
						<div className={`modal-header-custom`} style={{ backgroundColor: activePlan.color || "#0087B7" }}>
						...
						<div className='modal-body-custom'>
							<img src={activePlan.image || logoFallback} className='modal-img' alt={activePlan.name} />

							{activePlan.description && <p className='text-muted small mt-3 mb-3'>{activePlan.description}</p>}

							<div className='plan-list'>
								{(activePlan.featuresList || []).map((servicio, index) => (
									<div key={index} className='plan-list-item'>
										<span>{servicio}</span>
										<i className='fas fa-check-circle' style={{ color: activePlan.color || "#0087B7" }}></i>
									</div>
								))}
							</div>
